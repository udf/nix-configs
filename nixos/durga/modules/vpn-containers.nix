...
in
{
  ...
  config = (
    mkMergeTopLevel [ "users" "networking" "containers" ] (
      (mapAttrsToList (name: opts: {
        ...
        users =
          ...
          {
            users = (
              genAttrs userNames (u: {
                isSystemUser = true;
                group = u;
              })
            );
            groups = (genAttrs userNames (u: { }));
          };
        ...
        containers."${name}" = {
          ...
          config =
            { config, pkgs, ... }:
            {
              ...
              users = {
                users = (
                  genAttrs (flatten (attrValues opts.storageUsers)) (u: {
                    isSystemUser = true;
                    group = mkDefault u;
                  })
                );
                groups = mkMerge (
                  mapAttrsToList (dir: users: {
              ...
              networking = {
                ...
                firewall.allowedUDPPorts = [
                  vpnConsts.serverPort
                ]
                ++ (attrValues vpnConsts.clients."${name}".forwardedUDPPorts);
